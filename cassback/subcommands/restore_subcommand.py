...
class RestoreSubCommand(subcommands.SubCommand):
    ...
    def add_sub_parser(cls, sub_parsers):
        ...
        sub_parser.add_argument(
            "--threads", type=int, default=1, help='Number of writer threads.')
        sub_parser.add_argument(
    ...
    def __call__(self):
        ...
        for component in manifest.iter_components():
            msg = (json.dumps(manifest.serialise()),
                   json.dumps(component.serialise()))
            work_queue.put(msg)
            components.append(component)
        self.log.info("Queued components for restore: %s", ", ".join(
        ...
        while not result_queue.empty():
            op_results.append(
                RestoreResult.deserialise(
                    json.loads(result_queue.get_nowait())))

        str_bld = []
        failed_ops = [
            op_result for op_result in op_results if op_result.failed_reason
        ]

        skipped_ops = [
            op_result for op_result in op_results
        ...
        corrupt_files = [
            op_result for op_result in op_results if op_result.corrupt_path
        ]
        ...
        if corrupt_files:
            ...
            for op_result in corrupt_files:
                str_bld.append("%s -> %s" % (op_result.restore_path,
                                             op_result.corrupt_path))
            str_bld.append("")
    ...
    def _do_run(self):
        ...
        def safe_get():
            try:
                ...
                return (cassandra.KeyspaceBackup.deserialise(
    ...
    def __call__(self):
        ...
        if existing_md5 != self.backup_file.md5:
            ...
            try:
                ...
                self.log.error(
                    "Error deleting corrupt restored file %s",
                    op_result.restore_path,
                    exc_info=True)
