...
class PurgeSubCommand(subcommands.SubCommand):
    ...
    def add_sub_parser(cls, sub_parsers):
        ...
        sub_parser.add_argument("keyspace", help="Keyspace to purge from.")
    ...
    def _purge_manifests(self, endpoint, keep_manifests):
        ...
        keep_paths = frozenset(
            manifest.backup_path for manifest in keep_manifests)
            ...
            if manifest_path in keep_paths:
                ...
                deleted.append(
                    endpoint.remove_file(
                        manifest_path, dry_run=self.args.dry_run))
        return deleted
    ...
    def _purge_sstables(self, endpoint, kept_components):
        ...
        ks_dir = cassandra.BackupFile.backup_keyspace_dir(
            self.args.host, self.args.keyspace)

        set_kept_files = frozenset(
            component.backup_file_name for component in kept_components)
        ...
        for file_path in endpoint.iter_dir(ks_dir, recursive=True):
            ...
            if file_name in set_kept_files:
                ...
                deleted_files.append(
                    endpoint.remove_file_with_meta(
